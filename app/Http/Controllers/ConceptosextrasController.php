...
class ConceptosextrasController extends BaseController
{
    function mostrar(){
        try{
            $conceptos = Conceptosextra::where('eliminado', '=', '0')->get();
    ...
    function nuevo(Request $request){
        try{
            $concepto = Conceptosextra::create([
    ...
    function activar(Request $request){
        try{
            $concepto = Conceptosextra::find($request['id']);
            $concepto->activo = 1;
            $concepto->save();
